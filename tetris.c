...
int main(){
  ...
  while(1){
    afficher_fond(fond);
    switch(menu()){/*Le nombre que renvoie menu est la sélection du joueur*/
    case 1:
      partie=fopen("./Partie", "r+");
      if(sauvegarde_existe(partie)){/*On vérifie s'il y a une partie en mémoire*/
	afficher_fond(fond);
	reprendre=sous_menu_jouer();/*1 si le joueur veut reprendre sa partie*/
      }
      fini=0;
      switch(reprendre){
	...
	suivant=generer_piece();
	break;
      case 1:/*On charge la sauvegarde*/
	...
	while (J.pause){
	  ...
	  case 2:
	    ...
	    enregistrer_partie(J, partie, t, suivant, poche); /*On sauvegarde la partie dans le fichier*/
	    fclose(partie);
	    MLV_draw_text(50, 50, "Partie Sauvegardée", MLV_COLOR_WHITE); /*Pour indiquer au joueur que la partie a été sauvegardée*/
	...
	enregistrer_score(J, scores); /*On enregistre le score dans le fichier*/
	fclose(scores);
      }
      break;
    case 2:
      scores=fopen("./Scores", "r+");
      top_10(scores); /*On lit les 10 meilleurs scores dans le fichier*/
      fclose(scores);
      MLV_wait_seconds(5);
      break;
    case 3:
      fond=options(fond); /*On lance le menu des options, qui renvoie l'entier correspondant au fond que le joueur a choisi*/
      break;
    case 0:
      exit(EXIT_SUCCESS); /*On lance le jeu*/
      break;
    }
  }
}
